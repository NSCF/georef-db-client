<script>
  ...
  onMount((_) => {
    ...
    Auth.onAuthStateChanged(async (user) => {
      if (user) {
        ...
        if (window.location.href.includes('/auth/userMgmt')) {
          ...
          setTimeout(async (_) => {
            if (!profile) {
              //this is a sign in, fetch the profile
              try {
              ...
              try {
                ...
                if (snap.exists) {
                  ...
                  for (let datasetID of data.invited) {
                    const removeSearchEmail = Firestore.collection('datasets')
                      .doc(datasetID)
                      .update({ invitees: FieldValue.arrayRemove(profile.searchEmail) });
                    const addUserID = Firestore.collection('datasets')
                      .doc(datasetID)
                      .update({ invitees: FieldValue.arrayUnion(profile.uid) });
            ...
            await Firestore.collection('usersSignedIn')
              .doc('users')
              .update({ uids: FieldValue.arrayUnion(user.uid) });
            currentPage = 'Home'; // TODO this must go back to previous page the user was on
          }, 100);
        }
      } else {
    ...
    if (window.performance) {
      if (performance.navigation.type == performance.navigation.TYPE_RELOAD) {
        //deprecated but still works and WAY simpler than the new thing...
        if (window.location.href.includes('/auth/userMgmt')) {
          window.location = window.location.href.split('/auth/userMgmt')[0];
        }
      }
  ...
  function handleHomeClick() {
    if (currentPage == 'Georeferencer') {
      ...
      //we presume the user is trying to escape the manual georef
...
<svelte:window
  on:unload={async (_) =>
    await Firestore.collection('usersSignedIn')
      .doc('users')
      .update({ uids: FieldValue.arrayRemove(profile.uid) })}
/>
    ...
    <div class="main-flex-container">
      <div class="header">
        ...
        <div class="header-right">
          ...
          {:else if !profile}
            <div class="header-right">
              <button class="signin" on:click={(_) => (currentPage = 'Register')}
      ...
      <div class="content-container">
        ...
        {#if currentPage == 'ConfirmFields'}
          <ConfirmFields
            file={fileForGeoref}
            on:fields-confirmed={handleFieldsConfirmed}
            on:fields-confirm-cancelled={handleConfirmCanceled}
          />
        {/if}
        {#if currentPage == 'ConfirmData'}
          <ConfirmData
            file={fileForGeoref}
            {requiredFields}
            on:data-confirmed={handleFileContentsConfirmed}
            on:data-confirm-cancelled={handleConfirmCanceled}
          />
        {/if}
        {#if currentPage == 'RegisterDataset'}
          <RegisterDataset
            userProfile={profile}
            hasStateProvince={datasetSummary.hasStateProvince}
            datasetCountries={Object.keys(datasetSummary.localityRecordIDMap)}
            on:register-dataset={handleRegisterDataset}
          />
        {/if}
