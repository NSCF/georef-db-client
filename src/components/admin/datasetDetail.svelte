<script>
  ...
  onMount(async (_) => {
    ...
    if (
      dataset.createdByID == profile.uid ||
      (dataset.admins && dataset.admins.includes(profile.uid))
    ) {
      //we have admins, georeferencers, pastGeoreferencers, invited and newInvitees, and declinedInvitees

      let arrays = [
        dataset.admins,
        dataset.georeferencers,
        dataset.pastGeoreferencers,
        dataset.invitees,
        dataset.declinedInvitees,
      ];
  ...
  const getLocalDate = (timestamp) => {
    ...
    let dtParts = new Date(dt.getTime() - dt.getTimezoneOffset() * 60 * 1000)
      .toISOString()
      .split('T');
    return dtParts[0];
  };
  ...
  const addGeorefToRecord = (record, georef, georefDetails) => {
    ...
    let local = new Date(d.getTime() - d.getTimezoneOffset() * 60 * 1000)
      .toISOString()
    ...
    if (georef.locality && georef.locality.trim()) {
      //apparently sometimes it's not!
      if (dataset.localityField && record[dataset.localityField]) {
        localitySimilarity = stringSimilarity.compareTwoStrings(
          record[dataset.localityField],
          georef.locality
        );
      } else {
        let candidates = [
    ...
    if (origGeorefVals.length) {
      //this is an existing georef applied to locality string later
      ...
      //its the original georef but it may still have remarks
      if (georef.remarks && georef.remarks.trim()) {
    ...
    if (georefDetails.georefVerified) {
      if (georefDetails.georefVerifiedByRole) {
        record['dwc:georeferenceVerificationStatus'] =
          'verified by ' + georefDetails.georefVerifiedByRole;
      } else {
  ...
  const handleDownloadDataset = async (_) => {
    ...
    try {
      ...
      if (!georefIDs.length) {
        //there are no georefs for this dataset yet
        alert('there are no georeferences for this dataset yet, nothing will be downloaded');
        downloadProgessMessage = null;
        return;
      }
      //else
      let index = `${dataset.region.replace(/\s+/g, '')}${dataset.domain.replace(
        /\s+/g,
        ''
      )}`.toLowerCase();
  ...
  const handleCloseDataset = async (_) => {
    await Firestore.collection('datasets')
      .doc(dataset.datasetID)
      .update({ completed: true, dateCompleted: new Date().getTime() });
  ...
  const handleOpenDataset = async (_) => {
    await Firestore.collection('datasets')
      .doc(dataset.datasetID)
      .update({ completed: false, dateCompleted: null });
    ...
    if (recordGroupsQuerySnap.empty) {
      ...
      for (let recordGroup of recordGroups) {
        if (recordGroup.groupLocalities && recordGroup.groupLocalities.length) {
          for (let groupLoc of recordGroup.groupLocalities) {
            //this now has the georef fields
    ...
    if (conf) {
      ...
      if (list == 'georeferencers') {
        ...
        Realtime.ref(`stats/perDataset/${dataset.datasetID}/perUser/${uid}/georefsAdded`)
          ...
          .catch((err) => {
            console.error(
              'error checking dataset stats for georeferencer with uid',
              uid,
              ':',
              err.message
            );
  ...
  const handleProfileSelected = (ev) => {
    ...
    if (typeof item == 'string' && item.trim() && item.includes('invite')) {
      ...
      let exists = dataset.newInvitees.find(
        (x) =>
          x.replace(/[@\.\s]+/g, '').toLowerCase() == email.replace(/[@\.\s]+/g, '').toLowerCase()
      );
      if (!exists) {
        let searchEmail = email.replace(/[@\.\s]+/g, '').toLowerCase();
        let userDatasetsRef = Firestore.collection('userDatasets').doc(searchEmail);
        ...
        userDatasetsRef
          ...
          .catch((err) => {
            console.error('error updating userDatesets for', email, ':', error.message);
          });
      }
    } else {
    ...
    {:else if showDownloadComplete}
      <div class="vertical">
        <p style="margin-bottom:20px">
          The download has completed. Please remember to <span on:click={showUTFFilehint}
            >open the file in a UTF-8 friendly way...</span
          >
        </p>
        {#if downloadProblems.length}
          <p style="font-weight:bold;">
            There were problems with associating georeferences for the following {downloadProblems.length}
            record/s:
          </p>
    ...
    {:else}
      <div class="tools-container">
        ...
        {#if dataset.createdByID == profile.uid || (dataset.admins && dataset.admins.includes(profile.uid))}
          ...
          <button
            class="dataset-tool"
            title="download dataset with georefs"
            on:click={handleDownloadDataset}
          >
            <span class="material-icons">file_download</span>
          </button>
          <button
            class="dataset-tool"
            title="download georefs only"
            on:click={handleDownloadGeorefs}
          >
          ...
          <button
            ...
            on:click={(_) => dispatch('quality-control')}
            hidden={!(
              dataset.createdByID == profile.uid ||
              (dataset.admins && dataset.admins.includes(profile.uid))
            )}
          ...
          <button
            class="dataset-tool"
            title="clear locked record groups"
            on:click={clearLockedRecordGroups}
          >
        ...
        <button
          class="dataset-tool"
          title="start georeferencing"
          disabled={dataset.dateCompleted}
          on:click={handleStartGeoreferencing}
          bind:this={georefButton}
        >
      ...
      <div class="content">
        ...
        <div>
          ...
          <span class="data-item"
            >{dataset.dateCreated ? getLocalDate(dataset.dateCreated) : null}</span
          >
        </div>
        <div>
        ...
        <div>
          ...
          <span class="data-item"
            >{dataset.completed ? getLocalDate(dataset.dateCompleted) : 'NA'}</span
          >
        </div>
        <div>
        ...
        <div>
          ...
          <span class="data-item"
            >{dataset.lastGeoreference ? getLocalDateTime(dataset.lastGeoreference) : 'NA'}</span
          >
        </div>
        <div>
          <span class="label">Last georeference by</span>
          <span class="data-item"
            >{dataset.lastGeoreferenceBy ? dataset.lastGeoreferenceBy : 'NA'}</span
          >
        </div>
        {#if dataset.remarks}
          <div class="note">
      ...
      {#if dataset.createdByID == profile.uid || (dataset.admins && dataset.admins.includes(profile.uid))}
        {#if dataset.georeferencers && dataset.georeferencers.length}
          <div>
            ...
            <Team
              datasetCreatedByID={dataset.createdByID}
              ids={dataset.georeferencers}
              {profilesIndex}
              georef={true}
        ...
        {#if dataset.pastGeoreferencers && dataset.pastGeoreferencers.length}
          <div>
            ...
            <Team ids={dataset.pastGeoreferencers} {profilesIndex} on:reinvite={reInvite} />
          </div>
        {/if}
        {#if (dataset.invitees && dataset.invitees.length) || (dataset.newInvitees && dataset.newInvitees.length)}
          <div>
            ...
            <Team
              ids={[...dataset.invitees, ...dataset.newInvitees]}
              invitees={true}
              {profilesIndex}
              on:remove-user={removeUser}
