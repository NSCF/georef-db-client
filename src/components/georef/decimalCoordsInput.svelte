...
<div class="icon-input-container">
  <input
    ...
    class:hasError
    readonly
    on:paste={handleInputPasteCoords}
    bind:value
  />
  <span
    class="material-icons inline-icon icon-input-icon"
    style="right:30px"
    title="copy coords"
    on:click={copyDecimalCoords}>content_copy</span
  >
  <span
    class="material-icons inline-icon icon-input-icon"
    style="right:5px"
    title="paste coords"
    class:md-inactive={!editable}
    on:click={pasteDecimalCoords}>content_paste</span
  >
</div>
