<script>
  ...
  let validationVars = [
    ...
    'sourcesHasError',
    'verifiedByHasError',
    'verifiedDateHasError',
    'verifierRoleHasError',
  ]; //must match below

  $: hasLocalityAndCoords =
    editable &&
    Boolean(localGeoref.locality && localGeoref.locality.trim() && localGeoref.decimalCoordinates);
  $: localityHasError =
    editable &&
    Boolean(
      localGeoref.decimalCoordinates && (!localGeoref.locality || !localGeoref.locality.trim())
    );
  $: coordsHasError =
    editable &&
    Boolean(
      localGeoref.locality &&
        localGeoref.locality.trim() &&
        (!localGeoref.decimalCoordinates ||
          !localGeoref.decimalCoordinatesOkay ||
          localGeoref.decimalCoordinatesWarning) &&
        !localGeoref.ambiguous
    );
  $: uncertaintyHasError =
    editable &&
    Boolean(
      hasLocalityAndCoords &&
        requiredFields.includes('uncertainty') &&
        !localGeoref.uncertainty &&
        !localGeoref.ambiguous
    );
  $: uncertaintyUnitHasError =
    editable &&
    Boolean(localGeoref.uncertainty && !localGeoref.uncertaintyUnit && !localGeoref.ambiguous);
  $: datumHasError =
    editable &&
    Boolean(
      hasLocalityAndCoords &&
        requiredFields.includes('datum') &&
        (!localGeoref.datum || !localGeoref.datum.trim()) &&
        !localGeoref.ambiguous
    );
  $: georefByHasError =
    editable &&
    Boolean(
      hasLocalityAndCoords &&
        requiredFields.includes('by') &&
        (!localGeoref.by || !localGeoref.by.trim()) &&
        !localGeoref.ambiguous
    );
  $: georefDateHasError =
    editable &&
    Boolean(
      hasLocalityAndCoords &&
        ((localGeoref.by && localGeoref.by.trim()) || requiredFields.includes('date')) &&
        (!localGeoref.date || !localGeoref.date.trim()) &&
        !localGeoref.ambiguous
    ); //see the component for more validation
  $: protocolHasError =
    editable &&
    Boolean(
      hasLocalityAndCoords &&
        requiredFields.includes('protocol') &&
        (!localGeoref.protocolObject || !localGeoref.protocolObject.value) &&
        !localGeoref.ambiguous
    );
  $: sourcesHasError =
    editable &&
    Boolean(
      hasLocalityAndCoords &&
        requiredFields.includes('sources') &&
  ...
  $: verifiedByHasError =
    ...
    Boolean(
      hasLocalityAndCoords &&
        requiredFields.includes('verifiedBy') &&
        (!localGeoref.verifiedBy || !localGeoref.verifiedBy.trim())
    );
  $: verifiedDateHasError =
    editable &&
    Boolean(
      hasLocalityAndCoords &&
        ((localGeoref.verifiedBy && localGeoref.verifiedBy.trim()) ||
          requiredFields.includes('verifiedDate')) &&
        (!localGeoref.verifiedDate || !localGeoref.verifiedDate.trim())
    ); //see the component for more validation
  $: verifierRoleHasError =
    editable &&
    Boolean(
      hasLocalityAndCoords &&
        requiredFields.includes('verifierRole') &&
        (!localGeoref.verifierRole || !localGeoref.verifierRole.trim())
    );
  ...
  const setLocalGeoref = () => {
    ...
    if (georef && georef instanceof Georef) {
      ...
      if (defaultGeorefBy) {
        ...
        let now = new Date();
        localGeoref.date = new Date(now.getTime() - now.getTimezoneOffset() * 60 * 1000)
          .toISOString()
          .split('T')[0]; //we need this horrible thing to adjust for time zone differences as getTime gives a utc time
  ...
  const setDefaultORCID = (field) => {
    if (localGeoref[field] && localGeoref[field].trim()) {
      //make changes if there is a value
      if (defaultGeorefBy) {
  ...
  const updateOnLocalityChange = (_) => {
    if (localGeoref.locality) {
      //for updating verbatim coordinates if locality changes
      if (georef && (localGeoref.verbatimCoordinates || localGeoref.originalGeorefSource)) {
        ...
        if (localCleaned != originalCleaned) {
          ...
          localGeoref.uncertainty = null;
          localGeoref.uncertaintyUnit = null;
          localGeoref.datum = null;
          localGeoref.by = null;
          localGeoref.byORCID = null;
          localGeoref.date = null; // TODO set default date to today?
  ...
  $: showDuplicateWarning =
    isCreatingNewGeoref && hasExactMatchInCandidates && !localGeoref.ambiguous;
...
*/
  ...
  const handleClearClick = (_) => {
    if (georef && georef instanceof Georef) {
      //we got a georef from the parent
  ...
  const flagGeoref = (_) => {
    ...
    if (!localGeoref.flagged) {
      let conf = confirm('Are you sure you want to flag this georeference?');
      if (conf) {
  ...
  const handleCoordsFromVerbatim = (ev) => {
    try {
      ...
      if (
        localGeoref.decimalCoordinates &&
        localGeoref.decimalCoordinates.replace(/\s+/g, '') != coordsFromVerbatim
      ) {
  ...
  const handleSourceSelected = (ev) => {
    ...
    if (addedSource) {
      if (
        !georefSources
          .map((s) => s.value.toLowerCase().replace(/\s+/g, ' ').trim())
          .includes(addedSource.value.toLowerCase().replace(/\s+/g, ' ').trim())
      ) {
...
<form on:submit|preventDefault class:form-ambiguous={localGeoref.ambiguous}>
  <!-- georef tool buttons -->
  {#if showButtons}
    <div class="georef-tool-container">
      {#if showResetButton}
        <button
          type="button"
          class="georef-tool-button"
          title="Clear all"
          on:click|preventDefault={handleClearClick}
        >
      ...
      <button
        type="button"
        class="georef-tool-button"
        title="Flag this georef"
        disabled={localGeoref.flagged}
        on:click|preventDefault={flagGeoref}
      >
        <span class="material-icons">report</span>
      </button>
      <button
        type="button"
        class="georef-tool-button json-button"
        title="Copy georef JSON"
        on:click|preventDefault={copyGeorefJSON}
      >
        <span>JSON</span>
      </button>
      <button
        type="button"
        class="georef-tool-button"
        title="Copy tab delimited"
        on:click|preventDefault={copyTabDelimited}
      >
  ...
  <!-- georef ambiguous message -->
  {#if localGeoref.ambiguous}
    <div class="ambiguous-alert-container">
      <div class="ambiguous-alert">
        Please note this is a blank geoference used for imprecise, ambiguous, or otherwise
        ungeoreferencable locality strings. Any edits will update it to a regular georeference. Only
        edit if you understand the consequences for other locaties that might use it...
      </div>
    </div>
  {/if}
  ...
  <fieldset disabled={!editable}>
    ...
    <div class="oneliner">
      ...
      <VerbatimCoordsInput
        {editable}
        on:coords-from-verbatim={handleCoordsFromVerbatim}
        on:coords-from-paste={handleCoordsFromPaste}
        bind:value={localGeoref.verbatimCoordinates}
      />
    </div>
    <!-- decimal coords -->
    <div class="oneliner">
      ...
      <DecimalCoordsInput
        hasError={coordsHasError}
        {editable}
        on:coords-from-paste={handleCoordsFromPaste}
        bind:value={localGeoref.decimalCoordinates}
      />
      <!--VERY IMPORTANT THAT COORDINATES BE VALID BEFORE THIS HAPPENS, OTHERWISE IT BREAKS-->
    </div>
    <!-- metadata fields -->
    <fieldset class="innerfieldset" disabled={!metaEditable}>
      ...
      <div class="oneliner">
        ...
        <div class="uncertainty-fields">
          <input
            ...
            min="0"
            step="0.1"
            class:hasError={uncertaintyHasError}
            on:blur={handleUncertaintyBlur}
            bind:value={localGeoref.uncertainty}
          />
          <select
            class:hasError={uncertaintyUnitHasError}
            id="accunit"
            bind:value={localGeoref.uncertaintyUnit}
            bind:this={uncertaintySelect}
          >
      ...
      {#if showWKT}
        <div>
          <label for="WKT" style="width:100%;text-align:right"
            ><a href="https://dwc.tdwg.org/terms/#dwc:footprintWKT" target="_blank">georef WKT</a
            ></label
          >
      ...
      <div class="oneliner">
        ...
        <input
          id="datum"
          class:hasError={datumHasError}
          list="datums"
          name="datum"
          style="width:50%"
          bind:value={localGeoref.datum}
        />
        <datalist id="datums"> <option value="WGS84" /></datalist>
      </div>
      <!-- georef by and identifier -->
      {#if showGeorefBy}
        <div class="oneliner">
          <label for="georefBy">georef by</label>
          {#if defaultGeorefBy}
            <input
              class:hasError={georefByHasError}
              type="text"
              list="defaultGeorefBy"
              id="georefBy"
              style="width:70%"
              bind:value={localGeoref.by}
            />
            <datalist id="defaultGeorefBy"> <option value={defaultGeorefBy} /></datalist>
          {:else}
            <input
              class:hasError={georefByHasError}
              type="text"
        ...
        <div class="oneliner">
          ...
          <input
            ...
            bind:value={localGeoref.byORCID}
          />
        </div>
      {/if}
      <!-- georef date -->
      <div class="oneliner">
        <div class="fields">
          <div class="flex">
            ...
            <DateInput
              hasBy={localGeoref.by && localGeoref.by.trim()}
              {editable}
              hasError={georefDateHasError}
              bind:value={localGeoref.date}
            />
          </div>
        </div>
      </div>
      ...
      {#if editable}
        <div>
          ...
          <div class="inline-select">
            <Select
              items={georefProtocols.map((x) => ({ value: x, label: x }))}
              isCreatable={true}
              placeholder={'Select a protocol...'}
              hasError={protocolHasError}
              on:select={handleProtocolSelected}
              bind:value={localGeoref.protocolObject}
            />
          </div>
        </div>
      {:else}
      ...
      {#if editable}
        <div>
          ...
          <div class="inline-select">
            <Select
              items={georefSources.map((x) => ({ value: x, label: x }))}
              isCreatable={true}
              isMulti={true}
              placeholder={'Select source/s...'}
              hasError={sourcesHasError}
              on:select={handleSourceSelected}
              bind:value={localGeoref.sourcesArray}
            />
          </div>
        </div>
      {:else}
        <div class="oneliner">
          ...
          <textarea
            style="width:100%;max-width:300px;rows:2;resize:vertical;"
            bind:value={localGeoref.sources}
          />
        </div>
      {/if}
    </fieldset>
    ...
    <div>
      ...
      <textarea
        id="remarks"
        rows="3"
        disabled={!editable || !metaEditable}
        bind:value={localGeoref.remarks}
      />
    </div>
    <!-- georef verification -->
    {#if showVerification}
      ...
      {#if editable}
        ...
        <div>
          ...
          <textarea
            ...
            rows="3"
            bind:value={localGeoref.verificationRemarks}
          />
        </div>
        <div>
