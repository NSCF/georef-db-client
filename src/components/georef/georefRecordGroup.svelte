<script>
  ...
  const handleGroupLocClick = (ev, index) => {
    if (ev.ctrlKey) {
      $dataStore.recordGroup.groupLocalities[index].selected =
        !$dataStore.recordGroup.groupLocalities[index].selected;
    } else if (ev.shiftKey) {
  ...
  {#if $dataStore.recordGroup && !busy}
    {#each $dataStore.recordGroup.groupLocalities as groupLoc, i}
      <div hidden={groupLoc.georefID}>
        <div class="container">
          <p
            class="grouptext text-unselectable"
            class:selected={groupLoc.selected}
            on:click={(ev) => handleGroupLocClick(ev, i)}
          >
            {groupLoc.loc}
          </p>
          <p
            ...
            title="copy locality string"
            on:click={() => copyLocality(groupLoc.loc)}
          >
            content_copy
          </p>
        </div>
    ...
    {#if $dataStore.recordGroup.groupLocalities.filter((x) => x.georefID).length == $dataStore.recordGroup.groupLocalities.length}
      <h5>
        The localities for this group [{$dataStore.recordGroup.groupKey}] have been georeferenced
      </h5>
      <!--Show this when all georeferenced, needed for viewing a bookmarked record group-->
    {/if}
  {:else}
    <Loader />
  {/if}
