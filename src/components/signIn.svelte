    ...
    Auth.signInWithEmailAndPassword(email, pwd).catch((error) => {
      switch (error.code) {
        ...
        case 'auth/user-disabled':
          alert(
            `The profile for ${email} have been disabled. Please contact the NSCF for details.`
          );
          busy = false;
          break;
        case 'auth/user-not-found':
          alert(
            `We could not find a profile for ${email}. Please check your email or register if you don't have an account.`
          );
          busy = false;
          break;
        case 'auth/wrong-password':
          alert(
            `The password provided does not match the account for ${email}. Please check the password entered.`
          );
          busy = false;
          break;
        default:
  ...
  {:else}
    <form>
      ...
      <div style="text-align:center">
        <button
          style="margin:auto;width:200px;text-align:center;"
          on:click|preventDefault={handleSignInClick}>Sign in</button
        >
      </div>
    </form>
  {/if}
