        ...
        .catch((error) => {
          switch (error.code) {
            ...
            case 'auth/user-not-found':
              alert(
                `No user account could be found for ${email}. Please check the email provided or register a new account`
              );
              busy = false;
              break;
            default:
    ...
    <form>
      ...
      <div style="text-align:center">
        <button
          ...
          on:click|preventDefault={handleSendPwdCodeClick}>Send password reset code</button
        >
      </div>
    </form>
  {/if}
